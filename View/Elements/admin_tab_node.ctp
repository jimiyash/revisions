...
<?php
	...
	<?php
		...
		if (!empty($this->data['Node']['url']['type'])) {
			$nodeType = $this->data['Node']['url']['type'];
		} else {
			$nodeType = 'page';
		}
		...
		foreach ($this->data['Revisions'] as $revision) {
			...
			$revision['Revision']['title'] = $this->Html->link($revision['Revision']['title'], '/' . $nodeType . '/' . $this->data['Node']['slug'] . '/' . $revision_id, array('target'=>'_blank'));
